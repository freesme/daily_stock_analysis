...
def trigger_analysis(
    ...
    Query 参数:
        - code: 股票代码，如 600519
        - report_mode: 报告模式，可选值 full（完整版）或 compact（精简版），默认使用配置文件中的设置
    ...
    # 确定报告模式：优先使用请求参数，否则使用配置文件中的默认值
    config = get_config()
    effective_report_mode = report_mode if report_mode in ("full", "compact") else config.report_mode

    # 校验报告模式
    if report_mode and report_mode not in ("full", "compact"):
        raise HTTPException(status_code=400, detail="report_mode 参数无效，可选值: full, compact")

    # 使用后台任务异步执行完整分析流程（包括通知推送）
    background_tasks.add_task(trigger_single_stock_flow, code, effective_report_mode)

    logger.info(f"收到单股分析请求，已提交后台任务: {code}, 报告模式: {effective_report_mode}")
